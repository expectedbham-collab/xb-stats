...
def infer_label(history, teams):
    """Use modal games played across all teams as the gameweek label."""
    from collections import Counter
    played_counts = [
        stats.get("atk_played")
        for stats in teams.values()
        if stats.get("atk_played")
    ]
    if played_counts:
        modal_gw = Counter(played_counts).most_common(1)[0][0]
        return f"GW{modal_gw}"
    if not history: return "GW1"
    last = history[-1].get("label", "GW0")
    try:
        num = int(''.join(filter(str.isdigit, last)))
        return f"GW{num + 1}"
    except:
        return f"GW{len(history) + 1}"
...
def run(dry_run=False, force=False):
    ...
    # Dedup on gameweek label -- overwrite existing snapshot for same GW
